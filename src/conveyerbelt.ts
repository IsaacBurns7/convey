...
export class ConveryerBeltTile extends Tile {
    ...
    public input_direction: number;
    public output_direction: number;
    ...
    drawStraight(ctx: CanvasRenderingContext2D){
        ...
        let dirs: number[][] = rotate([[x1c,y1c],[x2c,y2c]],(theta)); // 0 rotation if 2 b/c default is east
        ...
        let offset_vector:number[] = [Math.cos(theta * -1), Math.sin(theta * -1)];
        offset_vector[0] *= (this.offset % (this.width / 2));
        offset_vector[1] *= (this.offset % (this.height / 2));

        //now just how to draw these...

        ctx.moveTo(this.x + x1 + offset_vector[0], 
            this.y + y1 + offset_vector[1]); //[0,0]
        ctx.lineTo(this.x + this.width/2 + offset_vector[0],  
            this.y + this.height/2 + offset_vector[1]); 
        ctx.lineTo(this.x + x2 + offset_vector[0],
            this.y + y2 + offset_vector[1]); //[0,1]
        ctx.stroke();
    }
}